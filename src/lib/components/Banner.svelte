...
<button
	class={clsx(
		'text-left w-full h-52 md:h-64 lg:h-80 overflow-hidden relative rounded-lg',
		$$props.class
	)}
	on:click={() => {
		currIndex + 1 >= anime.length ? (currIndex = 0) : currIndex++;
...
>
	<div class="absolute top-0 left-0 w-full h-full pointer-events-none -z-10">
		<img
			src={currAnime?.main_picture?.large ??
				'https://yugenanime.tv/static/img/161130007900716348.jpg'}
	...
	<div class="flex w-full h-full px-10 md:px-12 lg:px-20">
		<section class="text-xl my-auto pr-4 flex flex-col">
			...
			<p class="text-sm mt-4 hidden md:block lg:hidden">
				{currAnime?.synopsis?.substring(0, 150)}...
			</p>
			<a
